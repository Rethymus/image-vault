...
- Added the CI-environment override pitfall: a workflow fallback must not reintroduce the demo image origin after the local Worker build guard is fixed.
...
- 补充记录 CI 环境变量覆盖构建防线的问题：本地修复后，workflow fallback 也必须同步，不能重新引入演示图片 origin。
